...
from config import AWS_S3_BUCKET, AWS_REGION, AWS_S3_PREFIX
...
class S3Service:
    ...
    def load_items(self, project_key: str, item_type: str = None, 
        ...
        # Se o usuário passou since_date, monte um prefixo até o dia
        if since_date:
    ...
    def get_item_history(self, project_key: str, days: int = 30, limit: int = 50):
        since_date = (datetime.utcnow() - timedelta(days=days)).replace(
            hour=0, minute=0, second=0, microsecond=0
        )
        prefix = f"{self.prefix.rstrip('/')}/{project_key}/"

        paginator = self.s3.get_paginator('list_objects_v2')
        page_it = paginator.paginate(Bucket=self.bucket, Prefix=prefix)
        ...
        for page in page_it:
            for obj in page.get('Contents', []):
                # extrai data da key e converte
                parts = obj['Key'].split('/')
                # parts = ['contexts', 'SAM', 'stories', '2025','05','09','uuid.json']
                date_str = '/'.join(parts[-4:-1])         # '2025/05/09'
                obj_date = datetime.strptime(date_str, '%Y/%m/%d')
                if obj_date < since_date:
                    continue
                item_type = parts[-5]                     # 'stories'
                history.setdefault(item_type, []).append(self.load_item(obj['Key']))
                count += 1
                if count >= limit:
                    break
            if count >= limit:
        ...
        # opcional: ordenar cada lista por created_at
